...
function CompletionRing({ percentage, size = 24, strokeWidth = 3 }: { percentage: number; size?: number; strokeWidth?: number }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      {isComplete && (
        <CheckCircle className="absolute w-3 h-3 text-green-500" />
      )}
    </div>
  );
}
  ...
  const handleNextStep = () => {
    ...
    if (!canSubmitProposal) {
      toast({
        ...
        description: "Please complete all required fields for all members before submitting.",
        variant: "destructive",
      });
      return;
    }
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6 border-b border-slate-200 pb-0">
          {group.members.map((member, index) => {
            const memberCompletion = getMemberCompletion(member, loanDetailsByMember[member.id]);
            return (
              <div
                key={member.id}
                onClick={() => setActiveMemberId(member.id)}
                data-testid={`member-tab-${member.id}`}
                className={cn(
              ...
              >
                <div className="flex items-center gap-2">
                  <CompletionRing percentage={memberCompletion.percentage} size={20} strokeWidth={2} />
                ...
                <span className={cn(
                  ...
                  memberCompletion.percentage === 100 ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"
                )}>
                  {memberCompletion.percentage}%
                </span>
                <button 
                  onClick={(e) => handleRemoveMember(member.id, e)}
                  className="ml-1 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
        ...
        <div className="mb-4 border-b border-slate-200 flex gap-4">
          {[
            ...
            const pageCompletion = activeMember 
              ? getPageCompletion(activeMemberId!, section.id as PageType, activeMember, loanDetailsByMember[activeMemberId!])
              : { percentage: 0 };
            return (
              <button
                key={section.id}
              ...
              >
                <CompletionRing percentage={pageCompletion.percentage} size={18} strokeWidth={2} />
                <span>{section.label}</span>
                <span className={cn(
                  "text-[10px] font-bold px-1.5 py-0.5 rounded",
                  pageCompletion.percentage === 100 ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"
                )}>
        ...
        <div className="mt-8 flex justify-between items-center">
          <Button variant="outline" className="h-12 px-8 font-semibold" onClick={handleBackToHome} data-testid="button-back-home">
